...
void compute_flux(int nelr, int* elements_surrounding_elements, double* normals, double* variables, double* fluxes)
{
	...
	for(int i = 0; i < nelr; i++)
	{
		...
		for (; j < upper_bound; j += stride)
		{
			...
			__m256d vec_block1 = _mm256_loadu_pd(&(normals[(i*NNB + j)*NDIM + 0]));
			__m256d vec_block2 = _mm256_loadu_pd(&(normals[(i*NNB + j)*NDIM + 4]));
			__m256d vec_block3 = _mm256_loadu_pd(&(normals[(i*NNB + j)*NDIM + 8]));
			// normal.x = normals[(i*NNB + j)*NDIM + 0];
			__m256d vec_normal_x = {vec_block1[0],
									vec_block1[3],
									vec_block2[2],
									vec_block3[1]};
			// normal.y = normals[(i*NNB + j)*NDIM + 1];
			__m256d vec_normal_y = {vec_block1[1],
									vec_block2[0],
									vec_block2[3],
									vec_block3[2]};
			// normal.z = normals[(i*NNB + j)*NDIM + 2];
			__m256d vec_normal_z = {vec_block1[2],
									vec_block2[1],
									vec_block3[0],
									vec_block3[3]};
			...
			else  // divergent case
			{
				// 1st unroll
				nb = vec_nb[0];
				normal.x = vec_normal_x[0];
				normal.y = vec_normal_y[0];
				normal.z = vec_normal_z[0];
				normal_len = vec_normal_len[0];
				...
				// 2nd unroll
				nb = vec_nb[1];
				normal.x = vec_normal_x[1];
				normal.y = vec_normal_y[1];
				normal.z = vec_normal_z[1];
				normal_len = vec_normal_len[1];
				if(nb >= 0) 	// a legitimate neighbor
				{
					density_nb =        variables[nb*NVAR + VAR_DENSITY];
					momentum_nb.x =     variables[nb*NVAR + (VAR_MOMENTUM+0)];
				...
				// 3rd unroll
				nb = vec_nb[2];
				normal.x = vec_normal_x[2];
				normal.y = vec_normal_y[2];
				normal.z = vec_normal_z[2];
				normal_len = vec_normal_len[2];
				if(nb >= 0) 	// a legitimate neighbor
				{
					density_nb =        variables[nb*NVAR + VAR_DENSITY];
					momentum_nb.x =     variables[nb*NVAR + (VAR_MOMENTUM+0)];
				...
				// 4th unroll
				nb = vec_nb[3];
				normal.x = vec_normal_x[3];
				normal.y = vec_normal_y[3];
				normal.z = vec_normal_z[3];
				normal_len = vec_normal_len[3];
				if(nb >= 0) 	// a legitimate neighbor
				{
					density_nb =        variables[nb*NVAR + VAR_DENSITY];
					momentum_nb.x =     variables[nb*NVAR + (VAR_MOMENTUM+0)];
